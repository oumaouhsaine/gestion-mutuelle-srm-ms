...
const DevisDentaire = () => {
  ...
  const columns = ['Matricule Agent', 'Bénéficiaire', 'Date Devis', 'Date Dépôt', 'Date Réponse', 'Etat Réponse', 'Montant', 'Précision'];
  ...
  const API_URL = 'http://localhost:8081/api/devis-dentaire';
  ...
  const handleAdd = () => {
    setCurrentDevis({ idBeneficiaire: '', dateDevis: '', dateDepot: new Date().toISOString().split('T')[0], dateReponse: '', etatReponse: isClient ? 'En attente admin' : 'En attente', montant: '', precision: '', observation: '' });
    if (isClient && myAgent) {
  ...
  const handleEdit = (item) => {
    ...
    setCurrentDevis({
      ...
      scan: item.idScan, // On remet le nom du fichier dans le champ scan
  ...
  const handleSendToAdmin = async (item) => {
    try {
      ...
      if (response.ok) {
        ...
        // Notify admin via notification endpoint (to display in admin UI)
        fetch('http://localhost:8081/api/notifications', {
          ...
          body: JSON.stringify({
            type: 'devis-dentaire',
            id: item.idDevis,
            message: `Nouveau devis dentaire soumis (ID: ${item.idDevis})`
          })
        });
      }
    } catch (error) {
      ...
      if (response.ok) {
        ...
        if (emailAgent) {
          fetch('http://localhost:8081/api/notifications/email', {
            ...
            showAlert('Dossier pris en charge et notification envoyée à l\'agent.', 'success');
          });
        }
      }
    } catch (error) {
  ...
  const handleScan = async (e) => {
    ...
    if (file) {
      ...
      try {
        ...
        showAlert("Erreur lors de l'analyse du document par l'IA.", 'error');
      }
    }
  };
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <GenericTable
        title="Gestion des Devis Dentaires"
        columns={columns}
        data={displayDevis}
          ...
          <div className="modal-content" style={{ maxWidth: '600px' }}>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                <div>
                  <label style={{ display: 'block', marginBottom: '5px', fontSize: '0.9rem', color: '#4a5568' }}>Précision (Détails soins)</label>
                  <input required type="text" value={currentDevis?.precision || ''} onChange={e => setCurrentDevis({ ...currentDevis, precision: e.target.value })} style={{ width: '100%', padding: '8px', border: '1px solid #e2e8f0', borderRadius: '6px' }} />
                </div>
                {!isClient && (
                  <div>
            ...
            <div style={{ padding: '2.5rem' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
                <div style={{ background: '#f8fafc', padding: '1.5rem', borderRadius: '12px' }}>
                  ...
                  <p style={{ margin: '5px 0', fontSize: '0.9rem' }}><strong>Précision:</strong> {viewDevis.precision}</p>
